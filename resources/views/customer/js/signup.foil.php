...
function populateFormViaAsn() {
    ...
    const input_asn_search = $( '#asn-search' ).val().replace(/^[a-z]*/gi, '');

    if( input_asn_search && /^\s*\d+\s*$/.test( input_asn_search ) ) {

        let peering_policy = '';
        let url = " <?= url( "/customer/signup/query-peeringdb/asn" ) ?>/" + input_asn_search.trim();
